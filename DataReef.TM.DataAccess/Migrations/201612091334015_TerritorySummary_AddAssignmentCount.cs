namespace DataReef.TM.DataAccess.Migrations
{
    ...
    public partial class TerritorySummary_AddAssignmentCount : DbMigration
    {
        public override void Up()
        {
            Sql(@"ALTER procedure [dbo].[proc_TerritoryAnalytics]
                (
                ...
                FROM Properties p
                WHERE p.TerritoryID = @TerritoryID 
                AND (select top 1 pa.DisplayType from PropertyAttributes pa where PropertyId = p.Guid and AttributeKey in ('prescreen', 'prescreen-instant', 'prescreen-instant-Spruce') and IsDeleted = 0 and DateAdd(minute, ISNULL(ExpiryMinutes, 0), DateCreated) > getdate() order by DateCreated desc) like 'star%'
